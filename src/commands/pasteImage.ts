...
export async function pasteImage(
  ...
  outputChannel.appendLine(
    `[pasteImage] triggered, remoteName: ${vscode.env.remoteName}, platform: ${process.platform}`,
  );
  ...
  try {
    ...
    if (message.includes("Unsupported platform")) {
      vscode.window.setStatusBarMessage(
        "Paste image not supported on this platform",
        3000,
      );
      return;
    }
    ...
    vscode.window.setStatusBarMessage(
      "Execution failed, see Copy Reference output for details",
      3000,
    );
    ...
    try {
      const psPath = execSync(
        'which powershell.exe 2>/dev/null || echo "NOT_FOUND"',
        { encoding: "utf8" },
      ).trim();
      outputChannel.appendLine(`[pasteImage] powershell.exe path: ${psPath}`);
    } catch {
      outputChannel.appendLine(
        `[pasteImage] powershell.exe path: <unable to resolve>`,
      );
    }
  }
}
