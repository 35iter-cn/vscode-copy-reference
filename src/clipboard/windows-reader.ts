...
export class WindowsClipboardReader {
  async read(outputChannel: vscode.OutputChannel): Promise<string> {
    outputChannel.appendLine('[pasteImage] Starting clipboard image read (Windows)...');
...
if ([System.Windows.Forms.Clipboard]::ContainsImage()) {
  ...
  Write-Output "NO_IMAGE"
    ...
    const { stdout, stderr, error } = await execPromise(
      `powershell -NoLogo -NonInteractive -EncodedCommand ${encodedCommand}`
    );
    if (error) {
